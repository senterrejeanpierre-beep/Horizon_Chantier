...
class HorizonChantierApp(tk.Tk):
    ...
    def rendements_chantier(self):
        ...
        def trouver_ligne(libelle):
            for row in range(1, ws.max_row + 1):
                val = texte(ws[f"M{row}"].value)   # <-- libellés en colonne M
                if libelle in val:
                    return row
            return None

        def lire_o(libelle):
            row = trouver_ligne(libelle)
            if not row:
                return 0
        ...
        def lire_p(libelle):
            ...
            if not row:
                return 0
            return nombre(ws[f"P{row}"].value)

        nom_chantier = self.tree.item(self.tree.selection()[0])['values'][0]
        date_du_jour = datetime.now().strftime("%d/%m/%Y")

        # -------- LECTURE FEUILLE RENDEMENT --------
        heures_soumission = lire_o("soumission heures")
        heures_avenants_plus = lire_o("heures accordes avenant en plus")
        heures_avenants_moins = lire_o("heures accordes avenant en moins")
        heures_corrigees = lire_o("corrigé en heures")

        heures_consommees = lire_o("heures consomm")
        jours_consommes = lire_o("délai consommé")
        montant_produit = lire_p("heures consomm")

        ecart_pct = lire_o("ecart en %")
        ecart_jours_plus = lire_o("ecart en jour en plus")
        ecart_jours_moins = lire_o("ecart en jour en moins")

        # -------- CALCULS --------
        euro_heure = montant_produit / heures_consommees if heures_consommees else 0
        euro_jour = montant_produit / jours_consommes if jours_consommes else 0

        ecart_heures = heures_corrigees - heures_consommees

        if ecart_heures > 0:
            lecture = "En gain"
            couleur_lecture = colors.HexColor("#5DAE8B")
        elif ecart_heures < 0:
            lecture = "En perte"
            couleur_lecture = colors.HexColor("#C75C5C")
        else:
            lecture = "Équilibre"
            couleur_lecture = colors.HexColor("#D9A441")

        pdf_path = chemin.replace("Etat_avancement_02.xlsm", "rendement_chantier.pdf")
        ...
        # -------- TITRE --------
        titre = Table([["RENDEMENT CHANTIER"]], colWidths=[140 * mm], rowHeights=[22 * mm])
        titre.hAlign = "CENTER"
        titre.setStyle(TableStyle([
            ("BACKGROUND", (0, 0), (-1, -1), colors.white),
            ("ALIGN", (0, 0), (-1, -1), "CENTER"),
            ...
            ("FONTNAME", (0, 0), (-1, -1), "Helvetica-Bold"),
            ("FONTSIZE", (0, 0), (-1, -1), 22),
            ("TEXTCOLOR", (0, 0), (-1, -1), colors.HexColor("#2F5D7C")),
            ("TOPPADDING", (0, 0), (-1, -1), 0),
            ("BOTTOMPADDING", (0, 0), (-1, -1), 0),
        ]))
        elements.append(titre)
        ...
        data = [
            ...
            ["Heures soumission", f"{heures_soumission:,.2f} h"],
            ["Avenants heures en plus", f"{heures_avenants_plus:,.2f} h"],
            ["Avenants heures en moins", f"{heures_avenants_moins:,.2f} h"],
            ["Heures corrigées", f"{heures_corrigees:,.2f} h"],

            ["Heures consommées", f"{heures_consommees:,.2f} h"],
            ["Jours consommés", f"{jours_consommes:,.2f} j"],
            ["Montant produit", f"{montant_produit:,.2f} €"],

            ["€/heure réel", f"{euro_heure:,.2f} €"],
            ["€/jour réel", f"{euro_jour:,.2f} €"],

            ["Écart heures", f"{ecart_heures:,.2f} h"],
            ["Écart jours en plus", f"{ecart_jours_plus:,.2f} j"],
            ["Écart jours en moins", f"{ecart_jours_moins:,.2f} j"],
        ...
        table.setStyle(TableStyle([
            ("BACKGROUND", (0, 0), (-1, 0), colors.HexColor("#2F5D7C")),
            ("TEXTCOLOR", (0, 0), (-1, 0), colors.white),
            ...
            ("BACKGROUND", (0, 1), (1, 4), colors.HexColor("#EAF2F8")),
            ("BACKGROUND", (0, 5), (1, 7), colors.HexColor("#EEF7EE")),
            ("BACKGROUND", (0, 8), (1, 9), colors.HexColor("#F8F5EA")),
            ("BACKGROUND", (0, 10), (1, 13), colors.HexColor("#F6F6F6")),

            ("TEXTCOLOR", (1, 4), (1, 4), colors.HexColor("#2F5D7C")),
            ("TEXTCOLOR", (1, 7), (1, 7), colors.HexColor("#5DAE8B")),
            ("TEXTCOLOR", (1, 8), (1, 9), colors.HexColor("#D9A441")),
        ...
        table.setStyle(TableStyle([
            ("TEXTCOLOR", (1, 13), (1, 13), couleur_lecture),
        ]))
