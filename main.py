...
def ouvrir_dossier(path: Path) -> None:
    """Ouvre un dossier dans l’explorateur (Mac/Windows/Linux)."""
    try:
...
class HorizonChantierApp(tk.Tk):
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Button(bas, text="🔄 Rafraîchir", command=self.refresh_liste).pack(side="left", padx=(0,10))
        ttk.Button(bas, text="✏️ Modifier", command=self.modifier_chantier).pack(side="left", padx=(0,10))
        ttk.Button(bas, text="🗑️ Supprimer", command=self.supprimer_chantier).pack(side="left")
    ...
    def ouvrir_fenetre_nouveau(self) -> None:
        ...
        ttk.Button(btns, text="💾 Enregistrer", command=lambda: self.enregistrer_depuis_fenetre(win)).pack(side="left", padx=(0, 10))
    ...
    def enregistrer_depuis_fenetre(self, win: tk.Toplevel) -> None:
        ...
        data = {
            ...
            "etat": "Devis",
            "avancement": 0,
            # On prépare la suite (on remplira plus tard)
            "materiaux": [],
            "main_oeuvre": [],
            "frais": [],
            "taches": [],
