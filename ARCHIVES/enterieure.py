...
class HorizonChantierApp(tk.Tk):
    ...
    def _build_ui(self) -> None:
        ...
        bar1 = ttk.Frame(root)
        bar1.pack(fill="x", pady=(5, 0))

        bar2 = ttk.Frame(root)
        bar2.pack(fill="x", pady=(2, 10))

       
        ttk.Button(bar1, text="➕ Nouveau chantier", command=self.nouveau_chantier).pack(side="left", padx=6)


        ttk.Button(bar1, text="📥 Importer bordereau", command=self.importer_bordereau).pack(side="left", padx=6)
        ttk.Button(
    bar1,
    text="📐 Postes / Métré",
    ...
    command=lambda: __import__("subprocess").run([
        ...
        ttk.Button(bar1, text="🔍 Vérifier PR", command=self.verifier_pr).pack(side="left", padx=6)
    ...
    command=lambda: __import__("subprocess").Popen([
        ...
        bar2 = ttk.Frame(root)
        bar2.pack(fill="x", pady=(5, 10))

        ttk.Button(bar2, text="⚙️ Calcul / Reca", command=self.calcul_reca).pack(side="left", padx=6)
        ttk.Button(bar2, text="📄 Ouvrir dans le logiciel", command=self.ouvrir_chantier).pack(side="left", padx=6)
        ...
        ttk.Button(bar2, text="📁 Ce chantier", command=self.dossier_du_chantier).pack(side="left", padx=6)
        ttk.Button(bar2, text="✏️ Modifier", command=self.modifier_chantier).pack(side="left", padx=6)
        ttk.Button(bar2, text="🗑️ Supprimer", command=self.supprimer_chantier).pack(side="left", padx=6)
        ttk.Button(bar2, text="🔄 Rafraîchir", command=self.refresh_liste).pack(side="left", padx=6)
        ttk.Button(
            bar2,
        ...
        ttk.Button(bar2, text="📊 Calcul état", command=self.calcul_etat_avancement).pack(side="left", padx=6)
        ttk.Button(bar2, text="♻️ Clôturer état", command=self.mise_a_zero_etat).pack(side="left", padx=6)
        ttk.Button(bar2, text="🧭 Pilotage", command=self.pilotage_chantier).pack(side="left", padx=6)
        ttk.Button(bar2, text="⏱ Rendements", command=self.rendements_chantier).pack(side="left", padx=6)
    ...
    def verifier_pr(self):
        ...
        if not chemin.exists():
            messagebox.showerror("PR", f"Fichier introuvable : {chemin}")
            return
    ...
    def pilotage_chantier(self):
        ...
        while ligne <= ws.max_row:
            j = ws[f"J{ligne}"].value or 0
            l = ws[f"L{ligne}"].value or 0
            t = ws[f"T{ligne}"].value or 0
            w = ws[f"W{ligne}"].value or 0

            try:
                total_marche += float(j) * float(l)
                total_realise += float(t)
                total_mois += float(w)
            except:
                pass
        ...
        messagebox.showinfo(
            "Pilotage chantier",
            f"Marché total : {total_marche:,.2f} €\n"
            f"Réalisé cumulé : {total_realise:,.2f} €\n"
            f"Mois en cours : {total_mois:,.2f} €\n"
            f"Avancement : {avancement:.2f} %"
        )
    ...
    def rendements_chantier(self):
        ...
        rendement_jour = 0 if jours == 0 else total_mois / jours
        part_mois = 0 if total_realise == 0 else (total_mois / total_realise) * 100

        messagebox.showinfo(
            "Rendements",
            f"Production du mois : {total_mois:,.2f} €\n"
            f"Production / jour : {rendement_jour:,.2f} €\n"
            f"Poids du mois : {part_mois:.2f} %"
        )
    ...
    def calcul_reca(self) -> None:
        p = self._chemin_chantier_selectionne()
        if not p:
           messagebox.showwarning("Mise à jour", "Sélectionne un chantier.")
           return
        ...
        dossier_chantier = p.parent / p.stem
        try:
            nb = inject_pv(dossier_chantier)
            sync_pv_to_copies(dossier_chantier)
            messagebox.showinfo(
                "Mise à jour",
                f"{nb} ligne mise à jour dans PV 2026.\nPuis PR importé dans l'état d'avancement.\nPuis PV synchronisé vers:\n- {PV_BACKUP}\n- {PV_CORRIGE}",
            )
        except Exception as e:
    ...
    def importer_pr_dans_etat(self):
        ...
        # PARAMÈTRES FIXES DU PR
        ligne_article_depart = 3
        ligne_prix_depart = 30
        decalage = 42

        bloc = 0

        while True:
            ligne_article = ligne_article_depart + bloc * decalage
            ligne_prix = ligne_prix_depart + bloc * decalage

            if ligne_article > ws_pr.max_row:
                break

            article = txt(ws_pr[f"B{ligne_article}"].value)
            prix = ws_pr[f"F{ligne_prix}"].value

            if article == "" and prix in (None, ""):
                break

            if article:
                pr_dict[article] = prix

            bloc += 1

        for ligne in range(24, ws_etat.max_row + 1):
            article_etat = txt(ws_etat[f"B{ligne}"].value)

        if article_etat in pr_dict:
            ws_etat[f"L{ligne}"] = pr_dict[article_etat]
